...
export interface IPostService {
  ...
  getUnpopularPosts (offset?: number): Promise<APIResponse<Post[]>>
...
export class PostService extends BaseAPI implements IPostService {
  ...
  public async getUnpopularPosts (offset?: number): Promise<APIResponse<Post[]>> {
    try {
      const accessToken = this.authService.getToken('access-token');
      const isAuthenticated = !!accessToken === true;
      const auth = {
      ...
      const response = await this.get('/posts/unpopular', { offset }, 
        isAuthenticated ? auth : null
      );
