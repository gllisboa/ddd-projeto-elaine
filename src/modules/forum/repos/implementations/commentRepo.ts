...
export class CommentRepo implements ICommentRepo {
  ...
  async getAverageCommentsPerMemberOnDate(date: Date | string): Promise<number> {
    const result = await this.models.sequelize.query(
      `SELECT AVG(comments_per_member) AS average_comments FROM (
        SELECT COUNT(*) AS comments_per_member
        FROM comment
        INNER JOIN member ON comment.member_id = member.member_id
        WHERE comment.created_at LIKE "%${date}%"
        GROUP BY comment.member_id
      ) AS comment_counts;`
    );
  
    const averageComments = result[0][0]?.average_comments || 0;
